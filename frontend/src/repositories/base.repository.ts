  ...
  implements IBaseRepository<T>
{
  ...
  async getSingle(
    ...
    if (!BaseSwitcher.isApolloRepository) {
      ...
      const query = this.getSingleQuery();
  ...
  async mutation(
    ...
    this.publicCall = isPublic;

    const response = this.mutate
      ? await this.mutate(mutation, variables, this.publicCall)
      : () => {
          throw new Error("Something went wrong with mutate state");
        };
    const key = `create${this.id.charAt(0).toUpperCase() + this.id.slice(1)}`;
    const result = response[key] || response.sendMessage; // Handle sendMessage specifically
    if (!result) throw new Error(`Failed to create ${this.id}`);
    return { result: this.model.deserialize(result) };
  }
